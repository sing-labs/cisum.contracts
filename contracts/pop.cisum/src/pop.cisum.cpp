...
void pop_cisum::mine(name payer, asset pay_amount, string memo)
{
    ...
    CHECKC(pay_amount.symbol == USDT_SYMBOL,         err::SYMBOL_MISMATCH, "pay_amount symbol must be USDT");
    ...
    // —— 确保全局资产的符号初始化正确 ——
    if (_gstate.available_rewards.symbol.code().raw() == 0) _gstate.available_rewards = asset(0, REWARD_SYMBOL);
    if (_gstate.issued_rewards.symbol.code().raw()    == 0) _gstate.issued_rewards    = asset(0, REWARD_SYMBOL);
    ...
    CHECKC(_gstate.max_rewards.symbol       == REWARD_SYMBOL, err::SYMBOL_MISMATCH, "max_rewards symbol mismatch");

    // —— 价格：CISUM/USDT （1eCISUM 对应多少 USDT 的最小单位）——
    asset price = get_price_from_swap_as_asset(REWARD_SYMBOL, USDT_SYMBOL);
    CHECKC(price.is_valid() && price.symbol == USDT_SYMBOL, err::INVALID_FORMAT, "invalid price from swap");
    ...
    if (_gstate.receivable.symbol.code().raw() == 0) _gstate.receivable = asset(0, USDT_SYMBOL);
...
void pop_cisum::setmaxreward(const asset& max_rewards) {
    ...
    check(max_rewards.symbol == REWARD_SYMBOL, "symbol mismatch for max_rewards");
