...
void cvticket::create( const name& issuer, const int64_t& maximum_supply, const nsymbol& symbol, const string& token_uri, const name& ipowner )
{
   require_auth( issuer );
...
void cvticket::setnotary(const name& notary, const bool& to_add) {
   require_auth( _self );
...
void cvticket::notarize(const name& notary, const uint32_t& token_id) {
   require_auth( notary );
...
void cvticket::issue( const name& to, const nasset& quantity, const string& memo )
{
...
void cvticket::transfer( const name& from, const name& to, const vector<nasset>& assets, const string& memo  )
{
...
void cvticket::setcreator( const name& creator, const bool& to_add){
   require_auth( _self );
...
void cvticket::_creator_auth_check( const name& creator){
...
void cvticket::addwhitelist(const name& acct)
{
      require_auth( _self );
...
void cvticket::delwhitelist(const name& acct)
{
