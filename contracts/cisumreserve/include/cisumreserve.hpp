...
/**
 * cisumreserve 合约：接收 USDT -> 按 flon.swap 价格折算 -> 发 SING 给付款人，并记录订单
 *
 * 配置存于 global：
 *   - admin        : 管理员
...
// 例如：price.amount 表示 1 left_sym 需要多少 right_sym（放大了 10^left_precision）
inline asset get_price_from_swap(const symbol& left_sym,const symbol& right_sym)
{
    ...
    const asset& L = it->left_pool_quant.quantity;
    const asset& R = it->right_pool_quant.quantity;
    ...
    check(price_amount > 0, "invalid price");
