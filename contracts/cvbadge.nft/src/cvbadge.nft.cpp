#include <cvbadge.nft/cvbadge.nft.hpp>

namespace flon {


void cvbadge::create( const name& issuer, const int64_t& maximum_supply, const nsymbol& symbol, const string& token_uri, const name& ipowner )
{
   require_auth( issuer );
...
void cvbadge::setipowner(const uint64_t& symbid, const name& ip_owner) {
...
void cvbadge::settokenuri(const uint64_t& symbid, const string& url) {
...
void cvbadge::setnotary(const name& notary, const bool& to_add) {
   require_auth( _self );
...
void cvbadge::notarize(const name& notary, const uint32_t& token_id) {
   require_auth( notary );
...
void cvbadge::issue( const name& to, const nasset& quantity, const string& memo )
{
...
void cvbadge::retire( const nasset& quantity, const string& memo )
{
    auto sym = quantity.symbol;
   //  check( sym.is_valid(), "invalid symbol name" );
    check( memo.size() <= 256, "memo has more than 256 bytes" );
...
void cvbadge::transfer( const name& from, const name& to, const vector<nasset>& assets, const string& memo  )
{
...
void cvbadge::sub_balance( const name& owner, const nasset& value ) {
...
void cvbadge::add_balance( const name& owner, const nasset& value, const name& ram_payer )
{
...
void cvbadge::setcreator( const name& creator, const bool& to_add){
   require_auth( _self );
...
void cvbadge::_creator_auth_check( const name& creator){
...
void cvbadge::addwhitelist(const name& acct)
{
      require_auth( _self ); 
...
void cvbadge::delwhitelist(const name& acct)
{
