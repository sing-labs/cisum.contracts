                            ...
                            const vector<ticket_info>& tickets)
{
  ...
  for (const auto& ticket : tickets) {
    ...
    const nasset  qty{ static_cast<int64_t>(ticket.total_count), t_sym };

    // 免费票判定：金额是否为 0
    const bool is_free = (ticket.price.symbol.code() == POINT_SYMBOL_CODE);

    // (A) nftcreate
    CREATE_NFT(SHOW_CONTRACT,static_cast<int64_t>(ticket.total_count) * 10,t_sym,ticket.token_uri);
    ...
    // (C) 先铸到 show（保持你现有流程：再由 show 转到 grab）
    ...
    if (is_free) {
      ...
      uint64_t rush_sale_id = gstate.last_rush_sale_id+1;
      ...
      const std::string memo = "add:" + std::to_string(rush_sale_id)+":"+ std::to_string(show.show_id);

      ISSUE_TO_GRAB(SHOW_CONTRACT,
                    GRAB_CONTRACT,
                    ( nasset{ static_cast<int64_t>(ticket.total_count),nsymbol(ticket.ticket_id) } ),
                    memo);
    }
  }
}
